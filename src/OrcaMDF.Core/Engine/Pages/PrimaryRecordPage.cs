...
namespace OrcaMDF.Core.Engine.Pages
{
	internal class PrimaryRecordPage : RecordPage
	{
		...
		private void parseRecords()
		{
			...
			// This is madness! And as such, should only be attempted on user tables
			if (Header.ObjectID == 30)
			{
				...
				for (int i = 0; i < bytesWithPotentialGhostData.Length - 1; i++)
				{
					...
					try
					{
						...
						// If record is shorter than what's humanly possible, abort
						if (potentialRecord.RawBytes.Length < 9)
							continue;
