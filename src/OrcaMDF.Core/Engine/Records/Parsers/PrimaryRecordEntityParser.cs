			...
			foreach (var record in page.Records)
			{
				...
				try
				{
					foreach (DataColumn col in dataRow.Columns)
					{
						...
						if (!col.IsNullable && columnValue == null)
							throw new Exception("Column value parsed as null, but schema does not allow nulls");

						dataRow[col] = columnValue;
					}

					finalDataRow = dataRow;
				}
				catch (Exception ex)
				{
					Console.WriteLine("Error while reading column values from record: " + ex.Message);
				}

				if (finalDataRow != null)
					yield return finalDataRow;
			}
		}
